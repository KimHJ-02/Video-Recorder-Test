...
while True:
    ...
    if key == 32:
        ...
        if recording:
            ...
            fh, fw = frame.shape[:2]
            print(f"Starting recording. cap reported {width}x{height} @ {fps}fps, recording at {record_fps}fps, frame size {fw}x{fh}")

            # MP4 저장을 위해 mp4v 코덱 시도
            fourcc = cv.VideoWriter_fourcc(*'mp4v')
            out = cv.VideoWriter(filename, fourcc, record_fps, (fw, fh))
            if not out.isOpened():
